    ...
    <div className="relative">
      <Team
        ...
        className={cn(
          'w-full',
          isAdmin
            ? 'tablet:min-h-[151px] desktop:min-h-[239px] desktop:gap-6 desktop:rounded-[20px] desktop:p-8 desktop:[&_[role=progressbar]]:w-[calc(100%-32px)] min-h-[151px] gap-4 rounded-xl p-5'
      ...
      {isAdmin && (
        <button
          ...
          className="hover:bg-background-secondary hover:text-brand-primary focus-visible:ring-brand-primary text-icon-primary desktop:top-auto desktop:right-6 desktop:bottom-[50px] absolute top-5 right-5 z-10 flex size-7 items-center justify-center rounded-md transition-all duration-200 focus-visible:ring-2 focus-visible:outline-none active:scale-95"
      ...
      {isSettingsOpen && isAdmin && (
        <div className="border-border-primary bg-background-primary desktop:top-[190px] desktop:right-0 absolute top-[52px] right-2 z-20 w-[120px] overflow-hidden rounded-xl border py-2 shadow-lg">
          <button
            type="button"
