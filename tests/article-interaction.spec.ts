...
test('로그인 사용자가 게시글 작성, 좋아요, 댓글 등록을 할 수 있다', async ({ page }) => {
  ...
  await test.step('글쓰기 버튼을 클릭한다', async () => {
    await page.locator('button.fixed.bottom-10').click();
  ...
  await test.step('좋아요 버튼을 클릭한다', async () => {
    const likeButton = page.locator('main div.flex.w-full.justify-end >> button');
  ...
  await test.step('프로필 메뉴에서 로그아웃을 선택한다', async () => {
    await page.locator('aside div.border-t svg').click();
